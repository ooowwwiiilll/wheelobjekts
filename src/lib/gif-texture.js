// ─────────────────────────────────────────────────────────────────────────────
// Shared animated-GIF → three.js texture. Used by /infinite (small planes) and
// /verse (larger cards). THREE.TextureLoader only decodes a GIF's first frame,
// and copying a hidden <img> per frame is unreliable (browsers pause GIF playback
// for images that aren't actually painted). So we decode every frame up front
// with the WebCodecs ImageDecoder API and drive them on a timer into a
// CanvasTexture. tickGifs() advances all loaded GIFs once per render frame
// (cost O(#gifs), not O(#instances)).
// ─────────────────────────────────────────────────────────────────────────────
...
// getGifTexture(url, onReady, { maxSize })
//   maxSize — longest-edge px the frames are downscaled to (memory cap). Default
//   192 (infinite planes are tiny); /verse passes a larger value for crisper cards.
export const getGifTexture = (url, onReady, { maxSize = 192 } = {}) => {
...
// Advance every loaded GIF once per animation frame (called from a scene loop).
