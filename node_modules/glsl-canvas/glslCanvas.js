...
// modified the source material so it takes in a dom element param and can be started with a function call
const shaderWebBackground = {}

const startShader = ({ domElement, fragShader }) => {
  const t = (a, b) => {
  ...
  function D(a) {
    y(fragShader, "No frag shader found.")
    return fragShader
  }
  function E(a) {
    a += "Vertex"
  ...
  function F(a, b) {
    document.readyState !== "loading" ? b() : window.addEventListener(a, b)
  }
  class G {
    constructor(a, b, c, d) {
      this.g = c
  ...
  function H(a) {
    const b = { antialias: !1, depth: !1, alpha: !1 }
    try {
      return new I(a, b)
    } catch (c) {
  ...
  function J(a, b, c, d, l, f) {
    function h(e, m, n) {
      try {
        const k = p
        const q = k.gl
        const P = K(k, e, q.VERTEX_SHADER, m)
        const Q = K(k, e, q.FRAGMENT_SHADER, n)
        const v = q.createProgram()
        q.attachShader(v, P)
        q.attachShader(v, Q)
        q.linkProgram(v)
        const r = v
    ...
    const R = Object.keys(b).length - 1
    let S = 0
    for (const e in b) {
      if (S++ < R) {
...
export { startShader, shaderWebBackground }
